...
Пусть у нас есть настроенный сервер Ejabberd, нам нужно подключиться к такому же удаленному серверу

Узнаем ip-адреса удаленного сервера имеющего подключение по VPN и создаем маршрутизацию:

20) **`route add -net 192.168.6.23 netmask 255.255.255.255 gateway 172.16.5.8 (интерфейс подключения) eno3`**
...
Пингуем сервера: jabber.bereg.com и 10.3.0.2

25) **`ping jabber.bereg.com`**
26) **`ping 10.3.0.2`**

Обязательно говорим системному администратора отвечающего за сервер DNS центрального офиса, что мы хотим к нему подулючиться, так как нужно прописать адрес нашего сервера в свой DNS.
